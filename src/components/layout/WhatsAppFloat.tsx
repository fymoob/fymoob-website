...
export function WhatsAppFloat() {
  ...
  // Em /imovel/* e /empreendimento/* a sticky bar inferior ja oferece CTA
  // de contato no mobile — manter o flutuante alem dela duplica chamada e
  // briga visualmente (verde neon vs tema editorial premium do hub
  // Reserva Barigui). No desktop nao tem sticky bar, entao o flutuante
  // continua sendo o unico anchor de contato sempre-visivel.
  const hideOnMobile = isPropertyPage || isEmpreendimentoPage
  ...
  return (
    <a
      ...
      className={`fixed bottom-20 right-4 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-all duration-300 hover:scale-110 md:bottom-6 md:right-6 md:h-14 md:w-14 ${hideOnMobile ? "hidden md:flex" : ""}`}
    >
