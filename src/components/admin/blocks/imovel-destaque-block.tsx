...
export const imovelDestaqueBlock = createReactBlockSpec(
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      return (
        <div
          className="my-3 rounded-xl border border-brand-primary-muted bg-brand-primary-light/30 p-4"
        ...
        >
          ...
          {codigo ? (
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-slate-500">Código {codigo}</p>
    ...
    <div>
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-brand-primary-muted bg-white py-2 pl-9 pr-3 text-sm text-slate-900 outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20"
        />
        {pending && (
          <Loader2
            size={14}
      ...
      {results.length > 0 && (
        <ul className="mt-2 max-h-60 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-sm">
          {results.map((r) => (
            <li key={r.codigo}>
              <button
                type="button"
                onClick={() => onSelect(r)}
                className="block w-full px-3 py-2 text-left text-sm transition-colors hover:bg-brand-primary-light/40"
              >
