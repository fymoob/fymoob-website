...
export async function generateStaticParams() {
  ...
  for (const [slug, { finalidade }] of Object.entries(FINALIDADES)) {
    const { properties } = await getProperties({ tipo: "Casa", finalidade, limit: 1000 })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!fin) return {}
  const { properties } = await getProperties({ tipo: "Casa", finalidade: fin.finalidade, limit: 1000 })
