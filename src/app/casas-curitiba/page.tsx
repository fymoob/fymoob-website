...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    // Fase 19.P2.A.1 — usa generateLandingTitle com count (consolidado em seo.ts)
    title: generateLandingTitle("Casa", undefined, count),
...
export default async function CasasCuritibaPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const params = await searchParams
  const page = Math.max(1, parseInt(params.page || "1", 10) || 1)

  const [{ properties, total }, bairros, tipos, cidades, stats] = await Promise.all([
    getProperties({ tipo: "Casa", page, limit: PAGE_SIZE }),
    getAllBairros(),
    getAllTypes(),
    getAllCities(),
    getPropertyStats(),
  ...
  const faqQuestions = generateLandingFAQ("Casa", total, precoMin, precoMax)
    ...
    <>
      ...
      <div className="w-full bg-slate-50 px-4 py-8 md:px-12 lg:px-20 2xl:px-32">
        ...
        <h1 className="mt-2 font-display text-2xl font-bold text-neutral-900 sm:text-3xl">
          {total} Casas à Venda e Aluguel em {bairrosComCasa.length}+ Bairros de Curitiba
        </h1>
        ...
        <PropertyListingGrid properties={properties.map(p => projectForCard(p))} total={total} totalLabel="casas" cardContext="search" />
        <Pagination currentPage={page} totalPages={Math.max(1, Math.ceil(total / PAGE_SIZE))} basePath="/casas-curitiba" />
      </div>

      {/* Bloco SEO 1000 palavras — Fase 19.P0.4 */}
      <LandingSEOContent tipo="Casa" total={total} />
      ...
      <section className="bg-neutral-50 py-12 md:py-16">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <DynamicFAQ
            ...
            title="Perguntas frequentes sobre casas em Curitiba"
          />
        </div>
      </section>
