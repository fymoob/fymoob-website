...
import { Building2, MapPin, DollarSign, Home, Maximize, BedDouble, Car, ShowerHead, Phone, MessageCircle } from "lucide-react"
import {
...
/** Group units by type (Dormitorios + AreaPrivativa range) */
function groupUnitTypes(properties: Property[]) {
  ...
  for (const p of properties) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        properties: [],
        area: p.areaPrivativa || 0,
        dorms: p.dormitorios || 0,
        suites: p.suites || 0,
        banheiros: p.banheiros || 0,
        vagas: p.vagas || 0,
        precoMin: p.precoVenda || p.precoAluguel || 0,
        foto: p.fotoDestaque || "",
      })
    }
  ...
  return Array.from(groups.entries())
    .map(([key, g]) => ({ key, ...g, count: g.properties.length }))
...
export default async function EmpreendimentoPage({ params }: EmpreendimentoPageProps) {
  ...
  const realEstateSchema = {
    ...
    name: emp.nome,
    description: descricao || `${emp.total} unidades disponíveis no ${emp.nome} em ${bairros.join(", ") || "Curitiba"}`,
      ...
      <section className="bg-neutral-900">
        ...
        {heroPhotos.length > 0 && (
          <div className="mx-auto mt-4 max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {allPhotos.length > 5 && (
              <p className="mt-2 text-right text-xs text-neutral-500">{allPhotos.length} fotos disponíveis</p>
            )}
          </div>
        )}
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              {construtora && (
                <p className="mt-2 flex items-center gap-1.5 text-neutral-300">
                  <Building2 className="h-4 w-4" /> Construtora {construtora}
                </p>
              )}
              {bairros.length > 0 && (
                <p className="mt-1 flex items-center gap-1.5 text-neutral-400">
                  <MapPin className="h-4 w-4" /> {bairros.join(", ")}, Curitiba - PR
                </p>
              )}
              {endereco && (
              ...
              {situacao && (
                <span className="mt-3 inline-flex rounded-full bg-brand-primary/20 px-3 py-1 text-xs font-semibold text-brand-primary">
                  {situacao}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                ...
                <p className="text-xs text-neutral-400">{properties.length === 1 ? "Unidade" : "Unidades"}</p>
              </div>
              {precoMin && (
                <div className="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
              ...
              {areaMin && areaMax && (
                <div className="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                  ...
                  <p className="text-xs text-neutral-400">Área privativa</p>
                </div>
              )}
              {tipos.length > 0 && (
                <div className="rounded-xl bg-white/10 px-4 py-3 backdrop-blur">
                  <p className="text-sm font-bold text-white">{tipos.join(", ")}</p>
                  <p className="text-xs text-neutral-400">{tipos.length === 1 ? "Tipo" : "Tipos"}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 py-10 lg:grid-cols-[1fr_340px]">
          ...
          <div className="space-y-12">
            ...
            {descricao && (
              <section>
                <h2 className="font-display text-xl font-bold text-neutral-900">Sobre o {emp.nome}</h2>
            ...
            {/* Unit Types */}
            <section>
              <h2 className="font-display text-xl font-bold text-neutral-900">Tipos de unidades</h2>
              <p className="mt-1 text-sm text-neutral-500">
                <Link href={`/busca?empreendimento=${encodeURIComponent(emp.nome)}`} className="text-brand-primary hover:underline">
                  → Ver preços e unidades disponíveis
                </Link>
              </p>
                ...
                {unitTypes.map((ut) => (
                  <div key={ut.key} className="flex items-center gap-4 rounded-xl border border-neutral-200 p-4 transition hover:border-brand-primary/30 hover:shadow-sm">
                    ...
                    <div className="flex-1">
                      ...
                      {ut.precoMin > 0 && (
                        <p className="mt-2 text-sm font-bold text-slate-900">A partir de {formatPrice(ut.precoMin)}</p>
                      )}
                    </div>
                    <span className="shrink-0 rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-600">
                      {ut.count} {ut.count === 1 ? "unidade" : "unidades"}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {infraestrutura.length > 0 && (
              <section>
                <h2 className="font-display text-xl font-bold text-neutral-900">Infraestrutura do condomínio</h2>
                ...
                {properties.map((p) => (
                  <Link key={p.codigo} href={`/imovel/${p.slug}`} className="group overflow-hidden rounded-xl border border-neutral-200 transition hover:border-brand-primary/30 hover:shadow-md">
                    <div className="relative aspect-[4/3]">
                      ...
                      {p.lancamento && (
                        <span className="absolute top-3 left-3 rounded-full bg-brand-primary px-2.5 py-0.5 text-[10px] font-bold uppercase text-white">Lançamento</span>
                      )}
                    </div>
                    <div className="p-4">
                      ...
                      {(p.precoVenda || p.precoAluguel) && (
                        <p className="mt-2 text-base font-extrabold text-slate-900">{formatPrice(p.precoVenda || p.precoAluguel)}</p>
                      )}
                    </div>
                  </Link>
                ))}
            ...
            <div className="sticky top-24 space-y-4">
              <div className="rounded-2xl border border-neutral-200 bg-white p-6">
                ...
                <a
                  ...
                  className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#1da851]"
                >
                ...
                <a
                  ...
                  className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-neutral-200 px-4 py-3 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50"
                >
                ...
                <p className="mt-4 text-center text-[11px] text-neutral-400">
                  CRECI J 9420 · Atendimento seg-sex 8h30-17h
                </p>
              </div>
              ...
              {precoMin && (
                <div className="rounded-2xl bg-neutral-900 p-6 text-white">
                  ...
                  {precoMax && precoMax !== precoMin && (
                    <p className="mt-1 text-xs text-neutral-400">Até {formatPrice(precoMax)}</p>
                  )}
                </div>
              )}
            </div>
      ...
      <div className="fixed bottom-14 left-0 z-[100] w-full border-t border-neutral-200 bg-white px-4 py-3 shadow-[0_-2px_10px_rgba(0,0,0,0.08)] lg:hidden">
        <div className="mx-auto flex max-w-lg items-center gap-3">
          <div className="min-w-0 flex-1">
            {precoMin && (
              <>
                <p className="text-lg font-extrabold text-slate-900">{formatPrice(precoMin)}</p>
        ...
        <div className="mx-auto max-w-4xl space-y-12 px-4 sm:px-6 lg:px-8">
          <DynamicFAQ
            questions={(() => {
              ...
              return [
                {
                  question: `Quantas unidades estão disponíveis no ${emp.nome}?`,
                  answer: `O ${emp.nome} possui ${properties.length} ${properties.length === 1 ? "unidade disponível" : "unidades disponíveis"} no site da FYMOOB.${precoMin ? ` Preços a partir de ${formatPrice(precoMin)}.` : ""}`,
                },
                ...baseFaq.slice(1),
              ]
            })()}
