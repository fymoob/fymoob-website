...
export async function generateStaticParams() {
  ...
  for (const [slug, { finalidade }] of Object.entries(FINALIDADES)) {
    const { properties } = await getProperties({ tipo: "Terreno", finalidade, limit: 500 })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!fin) return {}
  const { properties } = await getProperties({ tipo: "Terreno", finalidade: fin.finalidade, limit: 500 })
