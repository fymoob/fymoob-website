...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `${properties.length} sobrados ${fin.label.toLowerCase()} em Curitiba.${precos.length > 0 ? ` A partir de ${formatPrice(Math.min(...precos))}.` : ""} FYMOOB Imobiliaria.`,
    alternates: { canonical: `/sobrados-curitiba/${slug}` },
  }
}
...
export default async function Page({ params }: PageProps) {
  ...
  const fin = FINALIDADES[slug as keyof typeof FINALIDADES]
  if (!fin) notFound()
  return <TipoFinalidadePage tipo="Sobrado" tipoPlural="Sobrados" tipoSlug="sobrados" finalidade={fin.finalidade} finalidadeSlug={slug} finalidadeLabel={fin.label} />
}
